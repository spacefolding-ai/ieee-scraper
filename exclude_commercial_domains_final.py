...
Final version - excluding 'ipg' to keep University of Perugia authors.
"""
...
COMMERCIAL_DOMAINS = [
    ...
    'opalrt',
    'plexim',
    'ipg-automotive',  # Specific pattern for IPG Automotive
    # 'ipg' - REMOVED to keep University of Perugia (unipg.it)
    'rtds',
    'speedgoat',
    'ni.com',
    'modelingtech',
...
def filter_commercial_authors(json_path, commercial_patterns):
    ...
    # Update or replace exclusion info in metadata
    if 'exclusions' not in data['metadata']:
        data['metadata']['exclusions'] = []
    
    # Remove any previous commercial domain exclusions
    data['metadata']['exclusions'] = [
        ex for ex in data['metadata']['exclusions'] 
        if 'commercial' not in ex.get('reason', '').lower()
    ]
    
    # Add new exclusion info
    data['metadata']['exclusions'].append({
        'date': datetime.now().isoformat(),
        'reason': 'Excluded authors with commercial email domains (University of Perugia kept)',
        ...
        'note': 'IPG pattern removed to preserve University of Perugia (unipg.it) authors'
    })
...
def main():
    ...
    print("FINAL VERSION - University of Perugia authors KEPT")
    print("="*80)
    ...
    print("\n⚠️  Note: 'ipg' pattern removed to keep University of Perugia (unipg.it)")
    print("    Only 'ipg-automotive' pattern will match actual IPG Automotive emails")
    ...
    print(f"\n✓ University of Perugia authors KEPT (13 authors)")
